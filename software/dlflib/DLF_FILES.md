...
The `Logger` class, which all user interaction is performed with, manages one or more `Run` class instances. These are created/closed by functions called by the library user. IE a `Run` is created when a user calls `startRun()` on the `Logger` instance. Users interact with active runs through !!TODO.
