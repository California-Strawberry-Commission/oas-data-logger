...
enum dlf_stream_type_e : uint8_t { POLLED, EVENT };

/* Overall Header Definition (meta.dlf) */
struct dlf_meta_header_t {
  uint16_t magic = DLF_MAGIC;  // IDs DLF files. Also allows auto-detection of
                               // LSB/MSB encoding.
  dlf_time_t epoch_time_s;
  dlf_time_us_t tick_base_us;  // Base time interval, in us. Limits how fast
                               // samples will be stored.
  const char* meta_structure;  // Structure of metadata struct.
  uint32_t meta_size;  // Metadata size stored in case there is no metadata
                       // parser available
...
struct dlf_logfile_header_t {
  ...
  // Next: Array of dlf_polled_stream_header_segment_t OR
...
struct dlf_stream_header_t {
  ...
  const char*
      notes;  // Anything that needs to be communicated about this data stream.
  uint32_t type_size;  // Records the size of this type to allow parsing even
                       // when parser is not found.
} __attribute__((packed));
