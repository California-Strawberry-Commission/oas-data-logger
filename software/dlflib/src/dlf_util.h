...
/**
 * Copies a string to heap-allocated memory, and sets the passed pointer to said
 * memory. If heap memory already exists at str_dest, it is realloced.
 */
inline void str2heap(char **str_dest, const char *str) {
  ...
  *str_dest[0] = '\0';
  ...
  if (str != nullptr) strlcpy(*str_dest, str, s);
}
