...
void handleRunningState() {
  ...
  // const unsigned long UPLOAD_ATTEMPT_INTERVAL_MS = 30000; // Try upload every
  // 30s
  ...
  //     UploaderComponent tempUploader(SD_MMC, "/", UPLOAD_HOST, UPLOAD_PORT,
  //     tempOptions);
  //
  //     // The logger stores runs in the root directory with a specific naming
  //     convention
  ...
  //
  //       // Find the most recent run directory (which should be our current
  //       run)
  //       // Run directories don't have the lockfile when complete, but our
  //       current run
  ...
  //           File lockCheck = SD_MMC.open(String("/") + runDir.name() + "/" +
  //           LOCKFILE_NAME); if (lockCheck) {
  //
  //             lockCheck.close();
  ...
  //             runDir.name());
  //
  //             Serial.println("[Running State] Flushing data and finalizing
  //             headers for upload..."); logger.flush(runHandle);
  //
  //             // Upload this run
  //             String uploadPath = String("/api/upload/") + runDir.name();
  //             bool uploadSuccess = tempUploader.uploadRun(runDir,
  //             uploadPath);
  //
  //             if (uploadSuccess) {
  //               Serial.println("[Running State] Successfully uploaded current
  //               run data");
  //               // Note: We don't mark or delete the run since it's still
  //               active
  //             } else {
  //               Serial.println("[Running State] Failed to upload current run
  //               data");
  //             }
  //
